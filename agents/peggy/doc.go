...
//     compaction knobs, coding tools, MCP servers, channels,
//     permission tiers, and schedules).
//
// Peggy ships a single-prompt CLI, a Telegram channel, durable
// sqlite+FTS5 memory with curated remember/recall, opt-in local coding
// tools, configured MCP tool servers, scheduled/proactive runs, and a
// local HTTP+SSE daemon so terminal and Telegram clients can share one
// long-running Peggy process. Tracker:
...
// (Provider, Store, Compactor, Searcher, Executor, Permission); Peggy
// fills them in.
package peggy
